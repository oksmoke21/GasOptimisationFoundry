...
contract GasContract {
    ...
    modifier checkIfWhiteListed() {
        require(whitelist[msg.sender] > 0 && whitelist[msg.sender] < 4, InvalidTier());
        _;
    }
    ...
    function checkForAdmin(address _user) public view returns (bool admin_) {
        assembly {
            ...
            for { let i := 0 } lt(i, 5) { i := add(i, 1) } {
                if eq(sload(add(adminsStartingSlot, i)), _user) { admin_ := 1 }
            }
        }
    }
    ...
    function transfer(address _recipient, uint256 _amount, string calldata _name) public returns (bool status_) {
        require(balances[msg.sender] >= _amount, InsufficientBalance(msg.sender));
    ...
    function whiteTransfer(address _recipient, uint256 _amount) external checkIfWhiteListed() {
        ...
        require(balances[msg.sender] >= _amount, InsufficientBalance(msg.sender));
        require(_amount > 3, InvalidValue());
